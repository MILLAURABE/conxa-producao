...
function EnvioForm({
  ...
  const custSel = costureiras.find(
    (c) => String(c.id) === String(form.costureira_id)
  );
  const colSel = colaboradores.find(
    (c) => String(c.id) === String(custSel?.colaborador_id)
  );
  const totalCol =
  ...
  const handleProduto = (prod) => {
    const c = costureiras.find(
      (x) => String(x.id) === String(form.costureira_id)
    );
    const precoProd = c?.precos_modelo?.find((p) =>
      prod.toUpperCase().includes(p.modelo.toUpperCase())
    );
    const prodObj = produtos.find(
      (p) => p.nome.toUpperCase() === prod.toUpperCase()
    );
    ...
    <div style={{ display: 'grid', gap: 14 }}>
        ...
        >
          ...
          {costureiras.map((c) => {
            const col = colaboradores.find(
              (x) => String(x.id) === String(c.colaborador_id)
            );
            return (
      ...
      {custSel && (
        ...
        >
          ...
          {custSel.aparelhos?.length > 0 && (
            <div style={{ color: COLORS.mid }}>
              🔧 {custSel.aparelhos.join(', ')}
            </div>
          )}
      ...
      {custSel?.precos_modelo?.length > 0 && (
        ...
        >
          ...
          >
            Tabela de preços de {custSel.nome}
          </div>
      ...
      >
        <Inp
          ...
          onChange={(e) => set('data_envio', e.target.value)}
        />
        <Inp
        ...
        <Inp
          ...
          onChange={(e) => set('data_conferencia', e.target.value)}
        />
      </div>
      ...
      <div style={{ background: COLORS.sand, borderRadius: 10, padding: 14 }}>
        ...
        >
          ...
          <Inp
            ...
            onChange={(e) => set('pecas_boas', e.target.value)}
          />
          <Inp
            label="Peças ruins"
            type="number"
            value={form.pecas_ruins}
            onChange={(e) => set('pecas_ruins', e.target.value)}
          />
          <Inp
            label="Itacaré"
            value={form.itacare}
            onChange={(e) => set('itacare', e.target.value)}
          />
          <Inp
            label="Site"
            value={form.site}
      ...
      >
        ...
        >
          <Inp
            ...
            onChange={(e) => set('valor_unit', e.target.value)}
          />
          <Inp
            label="Qtd a pagar"
            type="number"
            value={form.qtd_pagamento}
            onChange={(e) => set('qtd_pagamento', e.target.value)}
          />
          <Sel
            label="Pagamento realizado?"
            value={form.pagamento_realizado}
            onChange={(e) => set('pagamento_realizado', e.target.value)}
          >
        ...
        <div style={{ background: '#EDE6F5', borderRadius: 10, padding: 14 }}>
          ...
          >
            <Inp
              ...
              onChange={(e) => set('valor_colaborador', e.target.value)}
...
function CostureiraForm({ inicial, colaboradores, produtos, onSave, onClose }) {
  ...
  const removePreco = (i) =>
    set(
      'precos_modelo',
      (form.precos_modelo || []).filter((_, idx) => idx !== i)
    );
  ...
  return (
    <div style={{ display: 'grid', gap: 14 }}>
      ...
      <Section title="🔗 Vínculo de Produção" color={COLORS.teal}>
        ...
        {form.vinculo === 'indireta' && colaboradores.length === 0 && (
          <div style={{ marginTop: 8, fontSize: 13, color: COLORS.warn }}>
            ⚠️ Cadastre um colaborador primeiro.
          </div>
        )}
      </Section>
      ...
      >
        <div style={{ fontSize: 12, color: COLORS.muted, marginBottom: 10 }}>
          Defina preços específicos por modelo. No envio, o valor será
          preenchido automaticamente mas pode ser editado.
        </div>
        ...
        <div style={{ display: 'grid', gap: 14 }}>
          ...
          >
            <Sel
              ...
              onChange={(e) => set('corta_vies', e.target.value)}
            >
            ...
            <Inp
              ...
              onChange={(e) => set('largura_vies', e.target.value)}
              placeholder="Ex: 4mm, 3.5mm"
            />
            <Inp
              label="Média entrega 100 biquínis (dias) — manual"
              type="number"
              value={form.media_entrega_manual}
              onChange={(e) => set('media_entrega_manual', e.target.value)}
              placeholder="Ex: 15"
            />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <textarea
              ...
              onChange={(e) => set('obs_tecnicas', e.target.value)}
              rows={2}
              placeholder="Ex: Usa viés de 4mm no Carmel, 3mm no grosso..."
              style={{
                width: '100%',
                border: `1.5px solid ${COLORS.sandDark}`,
                borderRadius: 8,
      ...
      >
        <textarea
          ...
          onChange={(e) => set('obs_gerais', e.target.value)}
          rows={3}
          placeholder="Anotações gerais sobre a costureira..."
          style={{
            width: '100%',
            border: `1.5px solid ${COLORS.sandDark}`,
            borderRadius: 8,
...
function CostureiraCard({ c, col, envios, onEdit, onDelete, onVerPerfil }) {
  ...
  const tempos = envC
    .filter((e) => e.data_envio && e.data_conferencia)
    .map((e) => diffDias(e.data_envio, e.data_conferencia))
    .filter((d) => d > 0);
  const mediaAuto =
    tempos.length > 0
      ? Math.round(tempos.reduce((s, d) => s + d, 0) / tempos.length)
      : null;
  const mediaPor100 = mediaAuto
    ? Math.round(
        mediaAuto *
          (100 /
            (envC.reduce((s, e) => s + (parseFloat(e.quantidade) || 0), 0) /
              envC.length || 100))
      )
    : null;
    ...
    <Card>
      ...
      >
        <div style={{ textAlign: 'center' }}>
          ...
          {c.media_entrega_manual && mediaAuto && (
            <div style={{ fontSize: 10, color: COLORS.muted }}>
              Auto: {mediaAuto}d · Manual: {c.media_entrega_manual}d
            </div>
          )}
          {!c.media_entrega_manual && mediaAuto && (
            <div style={{ fontSize: 10, color: COLORS.muted }}>
              Calculado de {tempos.length} envio{tempos.length !== 1 ? 's' : ''}
            </div>
          )}
          {!mediaFinal && (
            <div style={{ fontSize: 10, color: COLORS.muted }}>
        ...
        {(c.precos_modelo || []).length > 0 && (
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
              ...
              {(c.precos_modelo || []).length > 4 && (
                <span style={{ fontSize: 11, color: COLORS.muted }}>
                  +{(c.precos_modelo || []).length - 4} mais
                </span>
              )}
            </div>
          </div>
...
function PerfilModal({ c, col, envios, onClose }) {
  const envC = envios
    .filter((e) => String(e.costureira_id) === String(c.id))
    .sort((a, b) => new Date(b.data_envio) - new Date(a.data_envio));
  const tempos = envC
    .filter((e) => e.data_envio && e.data_conferencia)
    .map((e) => ({
      dias: diffDias(e.data_envio, e.data_conferencia),
  ...
  const mediaAuto =
    tempos.length > 0
      ? Math.round(tempos.reduce((s, d) => s + d.dias, 0) / tempos.length)
      : null;
  const totalPago = envC
    .filter((e) => e.pagamento_realizado === 'sim')
    .reduce(
      (s, e) =>
        s +
    ...
    <Modal title={`Perfil — ${c.nome}`} onClose={onClose} wide>
      <div style={{ display: 'grid', gap: 16 }}>
        ...
        <Card style={{ background: COLORS.sand }}>
          <div style={{ fontWeight: 700, marginBottom: 10, color: COLORS.mid }}>
            🔧 Perfil Técnico
          </div>
          <div
            style={{
          ...
          >
            ...
            <div>
              <span style={{ color: COLORS.muted }}>Aparelhos:</span>{' '}
              <strong>{(c.aparelhos || []).join(', ') || '—'}</strong>
            </div>
            {c.obs_tecnicas && (
              <div style={{ gridColumn: '1/-1', color: COLORS.warn }}>
                ⚠️ {c.obs_tecnicas}
              </div>
            )}
            {c.obs_gerais && (
              <div style={{ gridColumn: '1/-1', color: COLORS.mid }}>
                {c.obs_gerais}
              </div>
            )}
          </div>
          ...
          <Card>
            ...
            <div style={{ marginBottom: 12, display: 'flex', gap: 16 }}>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                >
                  {c.media_entrega_manual || '—'}{' '}
                  {c.media_entrega_manual ? 'dias' : ''}
                </div>
              </div>
            </div>
            <table
            ...
            >
              <thead>
                <tr style={{ background: COLORS.sandDark }}>
                  {['Produto', 'Enviado', 'Conferência', 'Dias'].map((h) => (
                    <th
                      key={h}
                      style={{
                        padding: '6px 10px',
...
function Dashboard({ envios, costureiras, colaboradores }) {
  ...
  const emAndamento = envios.filter(
    (e) => e.situacao === 'Em andamento'
  ).length;
  const atrasados = envios.filter(
    (e) =>
      e.data_prevista && e.situacao !== 'OK' && new Date(e.data_prevista) < hoje
  ).length;
  const totalColAPagar = doMes.reduce((s, e) => {
    const c = costureiras.find((x) => String(x.id) === String(e.costureira_id));
    if (c?.vinculo !== 'indireta') return s;
    return (
      s +
      (parseFloat(e.pecas_boas) || 0) * (parseFloat(e.valor_colaborador) || 0)
    );
  }, 0);
  const totalColPago = doMes
    .filter((e) => e.pagamento_colaborador === 'sim')
    .reduce((s, e) => {
      const c = costureiras.find(
        (x) => String(x.id) === String(e.costureira_id)
      );
      if (c?.vinculo !== 'indireta') return s;
      return (
        s +
  ...
  const pendentes = envios
    .filter((e) => e.situacao !== 'OK' && e.situacao !== 'Cancelado')
    .slice(0, 8);
  return (
    <div style={{ display: 'grid', gap: 20 }}>
      ...
      >
        ...
        />
        <Stat
          label="A pagar colaborador"
          value={`R$ ${fmt(totalColAPagar)}`}
          color={COLORS.purple}
        />
        <Stat
          label="Pago colaborador"
          value={`R$ ${fmt(totalColPago)}`}
          color={COLORS.teal}
          sub={`Falta: R$ ${fmt(totalColAPagar - totalColPago)}`}
        />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        ...
        <Card>
          ...
          {doMes.filter((e) => e.pagamento_realizado !== 'sim').length === 0 ? (
...
function Envios({ envios, costureiras, colaboradores, produtos, setEnvios }) {
    ...
    () =>
      envios
        .filter((e) => {
          ...
          if (filtros.vinculo === 'direta' && c?.vinculo === 'indireta')
            return false;
          if (filtros.vinculo === 'indireta' && c?.vinculo !== 'indireta')
            return false;
          if (
            filtros.colaborador &&
            String(c?.colaborador_id) !== filtros.colaborador
          )
            return false;
  ...
  const salvar = (form) => {
    if (editando)
      setEnvios((prev) =>
        prev.map((e) =>
          e.id === editando.id ? { ...form, id: editando.id } : e
        )
      );
    else setEnvios((prev) => [...prev, { ...form, id: gerarId() }]);
  };
  const deletar = (id) => {
        ...
        >
          ...
          <div style={{ flex: '0 0 110px' }}>
            <Sel
              ...
              onChange={(e) =>
                setFiltros((f) => ({ ...f, tipo_produto: e.target.value }))
              }
            >
          ...
          {filtros.vinculo === 'indireta' && colaboradores.length > 0 && (
            <div style={{ flex: '0 0 140px' }}>
              <Sel
                label="Colaborador"
                value={filtros.colaborador}
                onChange={(e) =>
                  setFiltros((f) => ({ ...f, colaborador: e.target.value }))
                }
              >
                <option value="">Todos</option>
                {colaboradores.map((c) => (
                ...
                >
                  ...
                  <td style={{ padding: '9px 10px' }}>
                    {c?.vinculo === 'indireta' ? (
                      ...
                      <span className="tag tag-info">✅ Direta</span>
                    )}
                  </td>
                  ...
                  <td style={{ padding: '9px 10px', whiteSpace: 'nowrap' }}>
                    {e.quantidade} ·{' '}
                    <span style={{ color: COLORS.muted }}>{e.grade}</span>
                  </td>
                  <td
                    style={{
                      padding: '9px 10px',
...
function CostureirasList({
  ...
  const salvar = (form) => {
    if (editando)
      setCostureiras((prev) =>
        prev.map((c) =>
          c.id === editando.id ? { ...form, id: editando.id } : c
        )
      );
    else setCostureiras((prev) => [...prev, { ...form, id: gerarId() }]);
  };
  const deletar = (id) => {
    if (window.confirm('Remover esta costureira?'))
        ...
        >
          {[
            ...
            [
              'direta',
              `✅ Diretas (${
                costureiras.filter((c) => c.vinculo !== 'indireta').length
              })`,
            ],
            [
              'indireta',
      ...
      {perfil && (
        <PerfilModal
          ...
          col={colaboradores.find(
            (x) => String(x.id) === String(perfil.colaborador_id)
          )}
          envios={envios}
...
function Colaboradores({
  ...
  const salvar = (form) => {
    ...
    else setColaboradores((prev) => [...prev, { ...form, id: gerarId() }]);
  };
  const deletar = (id) => {
    ...
    <div style={{ display: 'grid', gap: 16 }}>
      ...
      {colaboradores.length === 0 && (
        <Card style={{ textAlign: 'center', padding: 48 }}>
          ...
          <div style={{ color: COLORS.muted, marginBottom: 20 }}>
            Colaboradores gerenciam costureiras indiretas. O valor por peça é
            definido em cada envio.
          </div>
          <Btn
            variant="purple"
            onClick={() => {
      ...
      >
        {colaboradores.map((col) => {
          const custsCol = costureiras.filter(
            (c) => String(c.colaborador_id) === String(col.id)
          );
          const enviosCol = envios.filter((e) =>
            custsCol.some((c) => String(c.id) === String(e.costureira_id))
          );
          const pecasBoas = enviosCol.reduce(
            (s, e) => s + (parseFloat(e.pecas_boas) || 0),
            0
          );
          const comissaoTotal = enviosCol.reduce(
            (s, e) =>
              s +
          ...
          return (
            ...
            >
              ...
              >
                ...
                <div style={{ fontSize: 12, color: COLORS.muted }}>
                  Valor por peça definido em cada envio
                </div>
              </div>
              <div
                style={{
              ...
              >
                ...
                {custsCol.length === 0 ? (
                  ...
                  custsCol.map((c) => (
                    <div key={c.id} style={{ fontSize: 13, padding: '2px 0' }}>
                      • {c.nome}{' '}
                      <span style={{ color: COLORS.muted }}>({c.tipo})</span>
                    </div>
                  ))
                )}
              </div>
...
function Relatorio({ envios, costureiras, colaboradores }) {
  ...
  const anos = [
    ...new Set(envios.map((e) => new Date(e.data_envio).getFullYear())),
  ].sort((a, b) => b - a);
  const doMes = useMemo(
    () =>
      envios.filter((e) => {
        const d = new Date(e.data_envio);
        return (
          d.getMonth() === parseInt(mes) && d.getFullYear() === parseInt(ano)
        );
      }),
    [envios, mes, ano]
  );
  const filtrarVista = (itens) =>
    itens.filter((e) => {
      const c = costureiras.find(
        (x) => String(x.id) === String(e.costureira_id)
      );
      if (vista === 'direta') return c?.vinculo !== 'indireta';
      if (vista === 'indireta') return c?.vinculo === 'indireta';
      if (vista !== 'todos') return String(c?.colaborador_id) === vista;
      return true;
    });
  const biquinis = filtrarVista(
    doMes.filter((e) => e.tipo_produto === 'Biquini')
  );
  const roupas = filtrarVista(doMes.filter((e) => e.tipo_produto === 'Roupa'));
  const outros = filtrarVista(
    doMes.filter(
      (e) =>
        e.tipo_produto &&
        e.tipo_produto !== 'Biquini' &&
        e.tipo_produto !== 'Roupa'
    )
  );
  const tudo = filtrarVista(doMes);
  const grandTotal = tudo.reduce(
    (s, e) =>
      s + (parseFloat(e.qtd_pagamento) || 0) * (parseFloat(e.valor_unit) || 0),
    0
  );
  const grandPecas = tudo.reduce((s, e) => s + (parseFloat(e.total) || 0), 0);
  const grandBoas = tudo.reduce(
    (s, e) => s + (parseFloat(e.pecas_boas) || 0),
    0
  );
  const comissoes = colaboradores
    .map((col) => {
      const custsCol = costureiras.filter(
        (c) => String(c.colaborador_id) === String(col.id)
      );
      const itensCol = tudo.filter((e) =>
        custsCol.some((c) => String(c.id) === String(e.costureira_id))
      );
      const comissao = itensCol.reduce(
        (s, e) =>
          s +
          (parseFloat(e.pecas_boas) || 0) *
            (parseFloat(e.valor_colaborador) || 0),
      ...
      const pecasBoas = itensCol.reduce(
        (s, e) => s + (parseFloat(e.pecas_boas) || 0),
        0
      );
      return { ...col, comissao, pecasBoas };
    })
    .filter((c) => c.pecasBoas > 0);
  ...
  function TabelaProdutos({ itens, titulo, cor }) {
    ...
    itens.forEach((e) => {
      ...
      pp[key].qtdEnviada += parseFloat(e.quantidade) || 0;
      pp[key].total += parseFloat(e.total) || 0;
      pp[key].pecasBoas += parseFloat(e.pecas_boas) || 0;
      pp[key].pecasRuins += parseFloat(e.pecas_ruins) || 0;
      if (c?.vinculo === 'indireta') {
        pp[key].indireta += parseFloat(e.pecas_boas) || 0;
        pp[key].colaborador = col?.nome || 'Indireta';
      } else pp[key].direta += parseFloat(e.pecas_boas) || 0;
    });
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        >
          ...
          <span style={{ fontSize: 13, fontWeight: 400, color: COLORS.muted }}>
            ({rows.length} modelos · {totBoas} peças boas)
          </span>
        </div>
        <table
          style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}
        >
          ...
          <tbody>
            {rows.map((r, i) => {
              const pct =
                r.total > 0 ? ((r.pecasBoas / r.total) * 100).toFixed(0) : 0;
              return (
                <tr
                  key={r.produto}
                  style={{
                ...
                >
                  ...
                  <td
                    style={{
                      padding: '8px 10px',
                      textAlign: 'center',
                      color: r.pecasRuins > 0 ? COLORS.danger : COLORS.muted,
                    }}
                  >
                    {r.pecasRuins || '—'}
                  </td>
                  ...
                  <td style={{ padding: '8px 10px', fontSize: 12 }}>
                    {r.indireta > 0 ? `${r.indireta} (${r.colaborador})` : '—'}
                  </td>
                </tr>
              );
            })}
            <tr style={{ background: COLORS.sandDark, fontWeight: 700 }}>
              <td style={{ padding: '8px 10px' }}>TOTAL</td>
              <td style={{ padding: '8px 10px', textAlign: 'center' }}>
                {rows.reduce((s, r) => s + r.qtdEnviada, 0)}
              </td>
              <td style={{ padding: '8px 10px', textAlign: 'center' }}>
              ...
              <td style={{ padding: '8px 10px', textAlign: 'center' }}>
                {rows.reduce((s, r) => s + r.pecasRuins, 0)}
              </td>
              <td colSpan={3}></td>
            </tr>
          </tbody>
    ...
    <div style={{ display: 'grid', gap: 20 }}>
      <Card style={{ padding: '14px 20px' }} className="no-print">
        ...
        >
          ...
          <Sel
            ...
            style={{ width: 210 }}
          >
            <option value="todos">Toda a produção</option>
            <option value="direta">✅ Somente Diretas</option>
            <option value="indireta">🔗 Somente Indiretas</option>
            {colaboradores.map((c) => (
              <option key={c.id} value={c.id}>
                👤 {c.nome}
              </option>
          ...
          {comissoes.map((col) => (
            ...
            >
              <div>
                ...
                <div style={{ fontSize: 12, color: COLORS.muted }}>
                  {col.pecasBoas} peças boas · valor por envio
                </div>
              </div>
              <div
                style={{ fontWeight: 700, color: COLORS.purple, fontSize: 16 }}
              >
                R$ {fmt(col.comissao)}
              </div>
            </div>
      ...
      <Card>
        ...
        {tudo.length === 0 && (
          ...
          >
            Nenhum envio registrado neste filtro.
          </div>
        )}
      </Card>
      {tudo.length > 0 && (
...
function Produtos({ produtos, setProdutos }) {
  ...
  const addProduto = () => {
    ...
    setProdutos((prev) => [
      ...prev,
      { nome: novo.nome.toUpperCase().trim(), cat: novo.cat },
    ]);
  ...
  const salvarEdit = (i) => {
    setProdutos((prev) => prev.map((p, idx) => (idx === i ? editando : p)));
    setEditando(null);
  };
      ...
      <Card>
        ...
        >
          <Inp
            ...
            placeholder="Ex: BOLSA SERENA, CINTO TRANÇA..."
          />
          <Sel
            label="Categoria"
            value={novo.cat}
            onChange={(e) => setNovo((f) => ({ ...f, cat: e.target.value }))}
          >
            {TIPOS_PRODUTO.map((t) => (
              <option key={t}>{t}</option>
            ))}
        ...
        return (
          <Card key={cat}>
            ...
            <div style={{ display: 'grid', gap: 6 }}>
              {itens.map((p, i) => {
                const realIdx = produtos.findIndex(
                  (x, xi) =>
                    x.nome === p.nome &&
                    x.cat === p.cat &&
                    xi === produtos.indexOf(p)
                );
                    ...
                    {editando && editando._idx === idx ? (
                      ...
                      >
                        ...
                        <select
                          value={editando.cat}
                          onChange={(e) =>
                            setEditando((f) => ({ ...f, cat: e.target.value }))
                          }
                          style={{
                            padding: '5px 10px',
                            borderRadius: 6,
                        ...
                        <Btn size="sm" onClick={() => salvarEdit(idx)}>
                          ✓ Salvar
                        </Btn>
                        <Btn
                          size="sm"
                      ...
                      <>
                        ...
                        <div style={{ display: 'flex', gap: 6 }}>
                          <Btn
                            size="sm"
                            variant="muted"
                            onClick={() => setEditando({ ...p, _idx: idx })}
                          >
                            ✏️
                          </Btn>
                          <Btn
      ...
      try {
        ...
        if (e.error || c.error || col.error || p.error) {
          ...
          if (Array.isArray(c) && c.length === 0) {
            for (const cost of COSTUREIRAS_INIT) {
              const { id, ...rest } = cost;
              await sbInsert('costureiras', rest);
            }
            const c2 = await sbGet('costureiras');
            setCostureiraRaw(Array.isArray(c2) ? c2 : []);
          }
          if (Array.isArray(p) && p.length === 0) {
            for (const prod of PRODUTOS_INIT_DEFAULT) {
              await sbInsert('produtos', prod);
            }
  ...
  const setEnvios = (fn) => {
    setEnviosRaw(async (prev) => {
      ...
      if (novo.length > atual.length) {
        ...
        // update
        const changed = novo.find(
          (n, i) => JSON.stringify(n) !== JSON.stringify(atual[i])
        );
        if (changed) {
          const { id, ...rest } = changed;
  ...
  const setProdutos = (fn) => {
    setProdutosRaw(async (prev) => {
      ...
      if (novo.length > atual.length) {
        ...
        if (changed) {
          const { id, ...rest } = changed;
          await sbUpdate('produtos', id, rest);
        }
        setProdutosRaw(novo);
      }
